<?php
 ...
 function __construct() {
  $widget_ops = array( 'classname' => 'tcdw_category_list_widget', 'description' => __('Displays Category list.', 'tcd-w') ); // Widget Settings
 ...
 function widget($args, $instance) {
  extract( $args );
   ...
   // Widget output //
   $cats = get_terms("category",'hide_empty=0&orderby=term_order&exclude='.$exclude_cat_num);
   if($cats){
?>
<ul<?php if($show_image != 1) { echo " class='no_image'"; }; ?>>
 <?php
      foreach ($cats as $cat):
        ...
        if(!empty($cat_data['category_image'])) {
          $image = wp_get_attachment_image_src( $cat_data['category_image'], 'thumbnail' );
        };
 ?>
 <li>
  <ul class="list_outer">
   <li class="list_image">
    <a class="image" href="<?php echo get_term_link( $cat_slug, 'category' ); ?>">
    <?php if ($show_image == 1) { ?>
    <?php if ($show_image == 1 && !empty($cat_data['category_image']) ) { ?><img src="<?php echo $image[0]; ?>" alt="" title="" /><?php } else { ?><img src="<?php echo bloginfo('template_url'); ?>/img/common/no_image1.gif" alt="" title="" /><?php }; ?>
    <?php }; ?>
    </a>
   </li>
   <li class="list_title">
    <a class="title" href="<?php echo get_term_link( $cat_slug, 'category' ); ?>">
   <?php echo $cat_name; ?>
    </a>
   </li>
  </ul>
